...
func GenToken(typ string, clientId int, clientName, clientPhone string, groupID int) (error, string) {
	claims := &types.JWTProfileClaims{
		AccountType: typ,
...
func GenPartnerToken(clientId, partnerId int, clientName string, groupID int) (error, string) {
	claims := &types.JWTProfilePartnerClaims{
		AccountType: "partner",
		ClientName:  clientName,
		ClientID:    clientId,
		PartnerId:   partnerId,
	...
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, claims)
	t, err := token.SignedString([]byte(fmt.Sprintf("%s", config.Config.Encryption.JWTSecretPartner)))
	if err != nil {
		return err, ""
	}
	return nil, t
...
func TenantLogin(phone, pass string) (*types.TenantLoginBody, error) {
	...
	var data types.TenantLoginBody
...
func TenantRegister(email, fullname, phone, pass string) (*types.TenantRegisterBody, error) {
	...
	defer res.Body.Close()
	body, _ := ioutil.ReadAll(res.Body)
	var data types.TenantRegisterBody
	err = json.Unmarshal(body, &data)
	if err != nil {
		return nil, err
	}
...
func TenantChangePass(oldPass, newPass string, id int) error {
	...
	if res.StatusCode != 200 {
		...
		var data types.TenantChangePassBody
		err = json.Unmarshal(body, &data)
		if err != nil {
			return err
		}
...
func Paging(page, total int) types.Paging {
	var pageInfo types.Paging
